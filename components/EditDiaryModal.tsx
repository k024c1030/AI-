...
const EditDiaryModal: React.FC<EditDiaryModalProps> = ({ entry, onSave, onClose }) => {
  const [plan, setPlan] = useState(entry.plan);
  const [checkedAchievements, setCheckedAchievements] = useState<Record<string, boolean>>(entry.achievements.predefined);
  const [customAchievements, setCustomAchievements] = useState<{ id: number; text: string }[]>(entry.achievements.custom);
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
        <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-md p-8 animate-fade-in-up">
            <button onClick={onClose} className="absolute top-4 right-4 text-slate-400 hover:text-slate-600" aria-label="Close diary editor">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                </svg>
            </button>
            <h2 id="diary-title" className="text-2xl font-bold text-slate-800 mb-4 text-center">日記を編集 ({entry.date.replace(/-/g, '/')})</h2>
            
            <form onSubmit={handleSubmit} className="space-y-6 max-h-[70vh] overflow-y-auto pr-2">
                {/* Plan */}
                <div>
                <label htmlFor="plan" className="block text-sm font-medium text-slate-700 mb-1">
                    <span className="font-bold text-teal-500">P (Plan)</span>: 明日のための小さな目標
                </label>
                <textarea
                    id="plan"
                    value={plan}
                ...
                <label className="block text-sm font-medium text-slate-700 mb-2">
                    <span className="font-bold text-teal-500">D (Do) / C (Check)</span>: できたこと
                </label>
                ...
                <button
                type="submit"
                className="w-full px-6 py-4 bg-teal-500 text-white rounded-xl font-semibold hover:bg-teal-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-teal-500 transition-colors duration-200 text-lg"
                >
                変更を保存する
                </button>
            </form>
